...
import { fetchJobApplications, type ApiJobApplication, type ApiJob } from '../../lib/api';
import { FileText, Mail, Phone, ExternalLink, Calendar, User } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Xem và quản lý các CV từ ứng viên gửi đến.</p>
        </div>
        <button
          onClick={loadData}
        ...
        <div className="grid gap-4">
          {applications.length === 0 ? (
            ...
            applications.map((app) => (
              <div key={app._id} className="bg-white rounded-xl shadow-sm border overflow-hidden hover:shadow-md transition-shadow">
                <div className="p-6">
                    ...
                    <div className="space-y-3 flex-1">
                      <div className="flex items-center gap-2">
                        <span className="inline-flex px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider bg-blue-50 text-blue-700 border border-blue-100">
                          {typeof app.jobId === 'string' ? 'Vị trí đã xoá' : (app.jobId as ApiJob).title}
                        </span>
                      ...
                      <div className="flex flex-wrap gap-4 text-sm text-gray-600">
                        <a href={`mailto:${app.applicantEmail}`} className="flex items-center gap-1.5 hover:text-blue-600 transition-colors">
                          <Mail className="w-4 h-4" />
                          {app.applicantEmail}
                        </a>
                        {app.applicantPhone && (
                          <a href={`tel:${app.applicantPhone}`} className="flex items-center gap-1.5 hover:text-blue-600 transition-colors">
                      ...
                      {app.coverLetter && (
                        <div className="bg-gray-50 rounded-lg p-4 text-sm text-gray-600 border border-gray-100 mt-2">
                          <p className="font-semibold text-gray-700 mb-1">Thư giới thiệu:</p>
